...
for prog_path in prog_lists:
    if 'trafficmonitor_path' not in locals().keys():
        if os.path.exists(os.path.join(prog_path, 'Traffic Monitor')):
            trafficmonitor_path = os.path.join(prog_path, 'Traffic Monitor')
...
try:
    ...
    remote_info = re.search(r'V([\d\.]*)', response.text, flags=re.M)
...
local_info = win.GetFileVersionInfo(os.path.join(trafficmonitor_path, 'TrafficMonitor.exe'), os.sep)
...
subprocess.call(['taskkill', '/F', '/IM', 'TrafficMonitor.exe'], stdout=open('NUL', 'w'), stderr=subprocess.STDOUT)
