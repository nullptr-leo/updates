...
for prog_path in prog_lists:
    if 'flowlauncher_path' not in locals().keys():
        if os.path.exists(os.path.join(prog_path, 'FlowLauncher')):
            flowlauncher_path = os.path.join(prog_path, 'FlowLauncher')
...
try:
    ...
    remote_info = re.search(r'Release v([\d\.]*)', response.text, flags=re.M)
...
local_info = win.GetFileVersionInfo(os.path.join(flowlauncher_path, 'Flow.Launcher.exe'), os.sep)
...
subprocess.call(['taskkill', '/F', '/IM', 'Flow.Launcher.exe'], stdout=open('NUL', 'w'), stderr=subprocess.STDOUT)
